...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in loop:
            ...
            # CRITICAL: Normalizing trajectories (same as training)
            mean_past_traj = past_loc.mean(dim = -2, keepdims = True)
            past_loc_normalized = past_loc - mean_past_traj
            fut_loc_normalized = fut_loc - mean_past_traj
            
            # prediction using model (with normalized inputs and correct signature)
            out = model(past_loc_normalized, None, unet_features, mode='testing')
            pred_traj_normalized = out[2]   # <-- pick the 3rd element (model_output)
            
            # CRITICAL: Denormalizing predictions (restore to original scale)
            pred_traj = pred_traj_normalized + mean_past_traj
            # Note: fut_loc remains unchanged for evaluation (original scale)
            
            # Evaluation metrics (using original scale trajectories)
