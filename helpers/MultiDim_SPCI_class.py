...
class SPCI_and_EnbPI():
    def __init__(self, X_train, X_predict, Y_train, Y_predict, fit_func):
        ...
        #Nile Added
        self.bootstrap_models = []  # Store actual fitted models
    ...
    def fit_bootstrap_models_online_multistep(self, B, stride=1):
        ...
        '''
        # CORRECTION 1: Clear previous bootstrap models to avoid accumulation on re-runs
        ...
        for s in range(stride):
            ...
            for b in range(B):
                ...
                #Nile Added: 
                model = copy.deepcopy(self.regressor) # Create independent copy 
                model.fit(Xboot, Yboot) # Fit on bootstrap sample 
                self.bootstrap_models.append(model) # Store the fitted model 
                boot_fX_pred = model.predict(Xfull) # Make predictions 
    ...
    #Nile added: 
    def save_bootstrap_models(self, filepath):
        """Save fitted bootstrap models to disk."""
        import pickle
        print(f"Saving {len(self.bootstrap_models)} bootstrap models to {filepath}...")
        with open(filepath, 'wb') as f:
            pickle.dump({
                'bootstrap_models': self.bootstrap_models,
                'train_centers': self.Ensemble_train_interval_centers,
                'pred_centers': self.Ensemble_pred_interval_centers,
                'residuals': self.Ensemble_online_resid,
                'global_cov': self.global_cov if hasattr(self, 'global_cov') else None,
                'global_cov_inv': self.global_cov_inv if hasattr(self, 'global_cov_inv') else None,
            }, f)
        print(f"Bootstrap models saved successfully!")

    def load_bootstrap_models(self, filepath):
        """Load fitted bootstrap models from disk."""
        import pickle
        print(f"Loading bootstrap models from {filepath}...")
        with open(filepath, 'rb') as f:
            data = pickle.load(f)
            self.bootstrap_models = data['bootstrap_models']
            self.Ensemble_train_interval_centers = data['train_centers']
            # CORRECTION 2: Complete restoration of all saved state
            self.Ensemble_pred_interval_centers = data['pred_centers']
            self.Ensemble_online_resid = data['residuals']
            if 'global_cov' in data and data['global_cov'] is not None:
                self.global_cov = data['global_cov']
                self.global_cov_inv = data['global_cov_inv']
        print(f"Loaded {len(self.bootstrap_models)} bootstrap models!")

    def predict_with_pretrained_models(self, X_new):
        """Make predictions using saved bootstrap models."""
        if not self.bootstrap_models:
            raise ValueError("No bootstrap models loaded. Call load_bootstrap_models() first.")
        ...
        # CORRECTION 3: Fixed indentation - return statement must be at method level
